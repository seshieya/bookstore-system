...
/*	public static Map<Long, Book> read() throws ApplicationException {
	...
	public static Map<Long, Book> read() throws ApplicationException {
		...
		File booksData = new File(FILENAME);
		
		FileReader in;
		Iterable<CSVRecord> records;
		try {
			in = new FileReader(booksData);
			records = CSVFormat.DEFAULT.withFirstRecordAsHeader().parse(in);
		} catch (IOException e) {
			throw new ApplicationException(e);
		}
		...
		LOG.debug("Reading" + booksData.getAbsolutePath());
		for (CSVRecord record : records) {
			long bookId = Long.parseLong(record.get("book_id"));
			String isbn = record.get("isbn");
			String authors = record.get("authors");
			int originalPublicationYear = Integer.parseInt(record.get("original_publication_year"));
			...
			try {
				book = new Book.Builder(bookId) //
						.setIsbn(isbn) //
						.setAuthors(authors) // 
						.setYear(originalPublicationYear)//
